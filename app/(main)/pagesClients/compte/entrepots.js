...
export default function EntrepotsPage() {
  ...
  const nearest = entrepots.reduce((prev, curr) => {
    const dPrev = distance(
      maison.latitude,
      maison.longitude,
      prev.latitude,
      prev.longitude,
    );
    const dCurr = distance(
      maison.latitude,
      maison.longitude,
      curr.latitude,
      curr.longitude,
  ...
  return (
    <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              ...
              style={[styles.btn, selectedId === item.id && styles.btnSelected]}
            >
      ...
      >
        <Marker coordinate={maison} title="Votre domicile" />
        ...
        <Polyline coordinates={routeMaison} strokeWidth={3} strokeColor="red" />
      </MapView>
    </View>
  );
}
